...
describe("Discover", function() {
    ...
    var expect = chai.expect;
    //var prefix = ["644", "645", "646", "647", "648", "649"];
    //var wordlength = [16, 19];
    ...
    it("has a prefix of 6011 and a length of 19", function() {
        expect(detectNetwork("6011345678901234564")).to.equal("Discover");
    });

    for (let prefix = 644; prefix <= 649; prefix++) {
        (function(prefix) {
            it("has a prefix of " + prefix + " and a length of 16", function() {
                expect(detectNetwork(prefix + "3456789051234")).to.equal(
                    "Discover"
                );
            });
            it("has a prefix of " + prefix + " and a length of 19", function() {
                expect(detectNetwork(prefix + "1345678901234358")).to.equal(
                    "Discover"
                );
            });
        })(prefix);
    ...
    //     it("has a prefix of " + prefix + " and a length of 16", function() {
    //         expect(detectNetwork(prefix[i] + "3456789051234")).to.equal(
    //             "Discover"
    //         );
    //     });

    //     it("has a prefix of " + prefix + " and a length of 19", function() {
    //         expect(detectNetwork(+prefix[i] + "134567890123435")).to.equal(
    //             "Discover"
    //         );
    //     });
    // }
    ...
    it("has a prefix of 65 and a length of 19", function() {
        expect(detectNetwork("6545987123456789234")).to.equal("Discover");
    });
});
...
describe("Maestro", function() {
    ...
    var ranNum = "123456789123456";

    for (let i = 8; i < 16; i++) {
        for (let j = 0; j < prefix.length; j++) {
            (function(prefix, i) {
                it(
                    "has a prefix of " + prefix + " and a length of " + (i + 4),
                    function() {
                        expect(
                            detectNetwork(prefix + ranNum.slice(0, i))
                        ).to.equal("Maestro");
                    }
                );
            })(prefix[j], i);
            ...
            (function(prefix, length) {
                it(
                    ...
                    function() {
                        expect(
                            detectNetwork(
                                (prefix + "123456789123456789").slice(0, length)
                            )
...
describe("should support Switch", function() {
    ...
    for (let el of [4903, 4905, 4911, 4936, 564182, 633110, 6333, 6759]) {
        for (let j of [16, 18, 19]) {
            (function(prefix, length) {
                it(
                    "has a prefix of " + prefix + " and a length of " + length,
                    function() {
